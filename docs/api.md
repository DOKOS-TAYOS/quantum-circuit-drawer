...
- `side.render.keyboard_shortcuts`: enable or disable managed keyboard navigation in `pages_controls` and `slider`, including arrows, `Home` / `End`, `PageUp` / `PageDown`, `Tab` / `Shift+Tab`, `Esc`, `0`, `?`, and `+/-` where supported; in `pages_controls`, `Up` adds one visible page, `Down` removes one, and `Tab` / `Shift+Tab` continue the column traversal across page boundaries
