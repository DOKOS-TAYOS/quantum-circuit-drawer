...
- `examples/logging_showcase.py`: turn on `configure_logging(...)`, compare `summary` / `detail` / `interactive`, and use `capture_logs(...)` to inspect the same run from code.
...
| `logging-showcase`                    | Structured logging profiles plus `capture_logs(...)` for programmatic inspection                                           | Windows and Linux            |
